...
impl ProofClient for FileBasedProofClient {
    ...
    fn pick_fri_job(&self) -> impl Future<Output = anyhow::Result<Option<(u32, Vec<u8>)>>> + '_ + Send {
        async move {
            ...
            let fri_job: Vec<u8> = serde_json::from_reader(file).context("Failed to parse fri_job.json")?;
            Ok(Some((block_number, fri_job)))
        }
    }

    fn submit_fri_proof(&self, _block_number: u32, proof: String) -> impl Future<Output = anyhow::Result<()>> + '_ + Send {
        async move {
            let path = self.base_dir.join("fri_proof.json");
            let mut file = std::fs::File::create(path).context("Failed to create fri_proof.json")?;
            serde_json::to_writer_pretty(&mut file, &proof).context("Failed to write fri_proof.json")?;
            Ok(())
        }
    }
    ...
    fn pick_snark_job(&self) -> impl Future<Output = anyhow::Result<Option<SnarkProofInputs>>> + '_ + Send {
        async move {
            ...
            let snark_job: SnarkProofInputs = serde_json::from_reader(file).context("Failed to parse snark_job.json")?;
            Ok(Some(snark_job))
        }
    }

    fn submit_snark_proof(&self, _from_block_number: L2BlockNumber, _to_block_number: L2BlockNumber, proof: SnarkWrapperProof) -> impl Future<Output = anyhow::Result<()>> + '_ + Send {
        async move {
            ...
            serde_json::to_writer_pretty(&mut file, &proof).context("Failed to write snark_proof.json")?;
            Ok(())
        }
    }
